...
uint32_t SSL_Get_IPv4(const char* hostname)
{
    ...
    LogInfo("DSN for %s: %u", hostname, result);
    return result;
}

int SSL_Socket_Create(uint32_t ipV4address, int port)
{
    int sock;
    int result = SSL_SOCKET_NULL_SOCKET;
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    int connect_result = connect(sock, (struct sockaddr*)&sock_addr, sizeof(sock_addr));
                    if (connect_result == -1)
                    {
                        ...
                        else
                        {
                            ...
                            int select_result = select(sock + 1, NULL, &writeset, &errset, &timeout);
                            if (select_result <= 0)
                            {
